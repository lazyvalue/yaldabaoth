//! Canonical on-disk locations for sketch's durable state.
//!
//! Everything sketch persists — the per-session WALs, the persisted session
//! list, workspace/preferences, the stable client id, and logs — lives under a
//! single home: `~/.sketch`. This is durable, user-owned storage, deliberately
...
//!
//! Config stays at `~/.config/sketch` (XDG, `config.rs`); runtime sockets stay
//! in `/tmp` (`session_proto::socket_path`). A one-time
//! [`migrate_legacy_cache_dir`] relocates state written by older builds under
//! `<cache_dir>/sketch` into `~/.sketch`, so upgrading loses nothing.

use std::path::PathBuf;

/// The single durable home for sketch state: `~/.sketch`. `None` only if the
/// user's home directory can't be resolved (then callers fall back to their
/// existing `Option`-`None` behavior — same as a first run).
pub fn sketch_home() -> Option<PathBuf> {
    dirs::home_dir().map(|h| h.join(".sketch"))
}

/// Legacy location used by builds before the `~/.sketch` move:
/// `<cache_dir>/sketch` (e.g. `~/Library/Caches/sketch`).
fn legacy_cache_dir() -> Option<PathBuf> {
    dirs::cache_dir().map(|d| d.join("sketch"))
}

/// One-time migration: move every entry from the legacy `<cache_dir>/sketch`
/// into `~/.sketch`, skipping any name that already exists in the new home (so
...
pub fn migrate_legacy_cache_dir() -> usize {
    let (Some(new_home), Some(old)) = (sketch_home(), legacy_cache_dir()) else {
        return 0;
    };
    if old == new_home {
        return 0;
    ...
    if moved > 0 {
        eprintln!(
            "[sketch] migrated {moved} state entr{} from {} to {}",
...
fn migrate_dir(old: &std::path::Path, new_home: &std::path::Path) -> usize {
    ...
    for entry in entries.flatten() {
        ...
        match std::fs::rename(&from, &to) {
            Ok(()) => moved += 1,
            Err(e) => eprintln!(
                "[sketch] could not migrate {} -> {}: {e} (left in place)",
                from.display(),
                to.display()
            ),
        }
...
mod tests {
    ...
    #[test]
    fn sketch_home_is_dot_sketch_under_home() {
        if let Some(home) = dirs::home_dir() {
            assert_eq!(sketch_home(), Some(home.join(".sketch")));
        }
    }

    /// Migration moves a legacy entry into a fresh `~/.sketch` but never
    ...
    fn migrate_moves_without_clobbering() {
        ...
        let base =
            std::env::temp_dir().join(format!("sketch_paths_{}_{}", std::process::id(), line!()));
        let old = base.join("Caches").join("sketch");
        let new = base.join(".sketch");
