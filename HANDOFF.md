...
| 5c | Doc/Edit single pooled `SharedCore` | 🔶 step 1 **on master** (`7b2c0f6`) | **Done + merged** (diag logs stripped): `DocState` binds a pooled `SharedCore` via `source: Option<DocSource>`; `edit_cache` retired; toggle shares the core; `refresh_blocks` re-derives Doc blocks live (read-only borrow — panic-safe); ALL Doc-open paths pool-bound (open_file, clone-for-split, restore, wiki-link, reload-in-place, startup). ⚠️ **Live tracking NOT yet runtime-verified** (merged on user request; builds + tests green + boots clean). **Step 2 (remaining):** cursor-stash on Doc→Edit (today cursor resets to top), save-clears-dirty-in-both, persist/restore correctness, gc/refcount hygiene. Per-keystroke Doc-side `render_with_wiki` re-parse cost (O(doc)) is unoptimized — separate from the Edit-side incremental reparse |
| 8b | delete turn-end inference in all 3 pumps | 🔶 stage 1 done | **Additive emit landed** (`8b-additive`, on master): `ReplyEvent::TurnEnded{count}` from the worker, gated `YALDA_EMIT_TURN_ENDED=1` (default off); GUI arm inert + logs agreement. **Stage 2 (delete inference) splits by risk** — investigated 2026-06-06: (i) **GUI-consumer half** — `apply_reply_events` finalizes on the explicit signal; delete the legacy direct (`gpui:12429`) + TUI (`claude.rs:326`) inferences. **Seam-test verifiable** (verify_harness drives `apply_server_batch`). Lower risk. (ii) **Server-pump half** — delete the inference at `session-server/main.rs:794`, rewire to the worker's explicit `TurnEnded`, **preserving the replay-fence (`:808-838`) + generation emit (`:856-871`)**. **NO headless coverage** (seam tests don't drive the server pump), storm-muddied resume path → needs a live **resume** runtime check; a fence mistake = transcript loss on reconnect. Do (i) anytime; do (ii) with the app + after/with the storm root-cause |
...
| Item | Status | Note |
|---|---|---|
| clippy `-D warnings` + `fmt --check` in CI | ⏸ | reality is **164 clippy + 531 fmt** diffs (not the stale "8"); dedicated passes only, and fmt would collide with the live `yalda-*` worktrees |
...
1. **Verify 5c live-tracking** at the app (5 min) — now **on master** (merged
   unverified on request). Two panes of one file, type in Edit → Doc updates;
   confirm no data loss on toggle. If broken, `git revert 7b2c0f6`. Then step 2.
