...
| **Tab** | `Tab<WindowContent>` | One tab-bar entry. Owns a layout tree and a focused-tile pointer. |
| **Split** | `Layout::Split` | Interior node in the layout tree — direction (`H`=stacked, `V`=side-by-side) + weighted children. |
| **Tile** | `Window<WindowContent>` (code name is `Window`) | A leaf in the split tree. Stable `WindowId` + content. |
| **Screen / Content** | `WindowContent` | What's inside a tile: `Doc`, `Edit`, `Agent`, `Browser`. |

Note: the code-level struct is still called `Window`, but in discussion we say
**tile** to avoid confusion with the OS-level frame.
