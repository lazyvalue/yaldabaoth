...
- [jump-panel.md](jump-panel.md) — `JumpPanel`. `UXI-JumpPanel-1..9` (the sidebar
  navigator plus its `Cmd-P` fuzzy palette, `UXI-JumpPanel-9`).
- [workspace.md](workspace.md) — `Workspace` (the infinite-plane model: signed
  ...
  dismissal on close). `UXI-Workspace-1..9`.
