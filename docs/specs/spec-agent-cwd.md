...
6. **Browser-window cwd is unrelated.** `BrowserWindow.fb.current_dir()` is the file browser's working directory, not the agent's. They may diverge — a user can browse `/foo` in one pane while their active agent operates on `/bar`. There is no link between the two in v1; a future "spawn agent at browser dir" affordance can land later without changing this spec.
