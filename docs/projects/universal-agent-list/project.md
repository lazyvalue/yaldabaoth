...
**Status:** 🟡 Phase 1 landed (jump panel reads a universal roster; builds + 223
tests green). Phase 2 (selector projection) planned, not started. Built on the
...
- **Phase 2 (planned):** selector renders/selects from the roster projection
  instead of its per-tile async list; retire `spawn_list_sessions_for_picker` /
  `apply_picker_sessions` (+ INV-PR routing). See ADR-0022 migration care.
...
| 002-ticket-selector-projection | render from roster · select from roster · next_agent_label from roster · migrate picker tests · update both-on-selection test | open |
