...
- [x] Create: ＋New project overlay (name + cwd) → `Projects::create`; dup name →
      transient error, nothing created; empty name cancels. New project is empty +
      persisted.
- [x] Per-project create: ＋New workspace → `new_workspace_in(project)`; ＋New
      agent session → `new_agent_session_in(project)` (cwd = project's, no prompt).
- [x] Delete: action + confirm overlay; non-empty → confirm → cascade (close
      workspaces, kill sessions) → remove; empty → delete directly. Empty projects
      persist otherwise.
- [x] Remove `RenameTarget::FreeAgentSessionCwd` + `AgentNewSessionCwd` cwd
      overlays and the global/`?`-menu "new agent session" cwd flow
      (`UXI-JumpPanel-3/4` superseded). Update those UXIs' status.
- [x] Guards: `new_project_overlay_creates_empty_project_and_rejects_dup`;
