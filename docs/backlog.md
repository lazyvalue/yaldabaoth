...
- **Autocomplete Cog Topic paths in Agent Tile inputs** — `NEEDS-RUNTIME` (2026-08-24
  ...
  no Cog-side feature request is needed. Follow-up adjustment (Cog graph `wiv`):
  in either Message Box or Worksheet Insert mode, a whitespace-delimited token
  beginning with `%` explicitly opens Topic completion; the suffix prefix-filters
  a freshly fetched live Topic catalog, arrows select, Tab/Enter replaces the
  whole `%…` token with the raw address, and Esc dismisses the current query. Each
  new `%` query refreshes once in the background, while suffix keystrokes reuse
  that query's result rather than spawning one process per character. Bare
  leading slash-command completion remains independent. Headless interaction and
  render probes pass in both placements; exact glyph/color appearance still needs
  a human runtime look.
...
- **Browse Cog by hierarchical topics and inspect agents' mail** — `NEEDS-RUNTIME`
  ...
  select graph, bulletin/note, or Chat detail in the right pane. A peer
  ...
  pending pixel/layout review in the native GPUI app. Root discovery and the
  new commands are deployed, and compatibility graph `cre` migrated the typed
  shared-communication leaf from Cog's removed `mailing_list` contract to its
  live `chat` contract (`cog chat get`; addresses, members, history). Live root
  and Chat payload probes pass; 704 GUI tests pass (2 ignored), the Chat-entry
  guard was observed RED then restored GREEN, and the release GUI builds.
...
- **Switch a workspace between plane and columns arrangements** — `DONE`
  (2026-08-12 via `/new-ux`; `UXI-Workspace-14`; Cog graph `snf`, default-policy
  follow-up `1i9` on 2026-08-16). Captured
  ...
  views."* A per-workspace `WorkspaceView { Plane, Columns }` (default `Columns`)
  ...
  `new_workspace_defaults_to_columns` pins the production creation path;
  ...
  plane culled — negative-control-verified RED.
  `workspace_view_round_trips_and_unknown_defaults_columns` pins explicit
  preferences plus absent/unknown fallback. Builds + 574 bin tests green.
