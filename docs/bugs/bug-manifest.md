...
| bug-0015 | code-block-shifts-under-the-pointer-on-click | FIXED | 2026-07-21 | 1 | "still can't select in a multiline code block" — bands + the selection/copy model path are both HEALTHY (bug-0008's fix is live); what is broken is that clicking inside a block sets the editor cursor, which collapses the cursor-kept blank line, drops a list item and repaints the block 25px lower (> one 20px line) under the pointer mid-drag. Fixed by freezing the collapse's protected line (`drag_protect_line`) for the duration of the drag; PAINT-level guard + NC (+25px) |
